...
**Status:** Removed (superseded by year composition only)
...
**B, hand-maintained file:** `data/metadata/country-colors.json` — flag-stripe averages generated once via `pipeline/scripts/generate_country_colors.py` (not run by `package`). Editors may edit hex values directly. `package` copies the file to `packaged/insights/country-colors.json`.

**`missing`:** `--chart-missing` theme token.

**`unknown`:** entry in `country-colors.json`.

**Page:** `/insights/country-composition/` (subpage; index links here).
...
3. Write `episode-country-composition.json`.
4. Copy `data/metadata/country-colors.json` → packaged (must exist).
...
`/insights/country-composition/` → `CountryCompositionInsight` island. Index at `/insights/` links here.
...
- [x] Packaged `episode-country-composition.json` generated by `evtop20 package`
- [x] `data/metadata/country-colors.json` hand-maintained; copied at package
- [x] `EpisodeCompositionChart` on `/insights/country-composition/`
- [x] Segment order: countries A→Z, gray missing rightmost
- [x] Colors stable per country; missing = gray
- [x] Pipeline + site tests green; `npm run build` green
- [x] `site/README.md` notes Insights data paths
